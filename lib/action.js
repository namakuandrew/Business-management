...
export async function getNextReferenceNo(companyId) {
  if (!companyId) {
    return { error: "Company Id is needed" };
  }
  const cookieStore = cookies();
  const supabase = createSupabaseServerClient(cookieStore);
  ...
  const prefix = company.reference_prefix;
  ...
  if (!latestEntry) {
    return { referenceNo: `${prefix}1` };
  }

  const lastRef = latestEntry.reference_no;
  const match = lastRef.match(/(\d+)$/);

  if (match) {
    const nextNumber = parseInt(match[1], 10) + 1;
    const newRef = lastRef.substring(0, match.index) + nextNumber;
    return { referenceNo: newRef };
  }
